		...
		<>
			{ data.map((item, index) => (
				<div key={ index } className="sm:rounded-lg">
					...
					<div className="bg-teal-100 dark:bg-teal-950 sm:p-8 p-4 rounded-lg fadeIn">
						<div
							className="flex flex-row justify-start gap-2 sm:justify-around sm:gap-0 overflow-x-auto sm:overflow-hidden">
							<SampleImg src={ item.imgDekstop } name={ item.name } aspect={ "aspect-[16/10]" }/>
							<SampleImg src={ item.imgTablet } name={ item.name } aspect={ "aspect-[18/22.5]" }/>
							<SampleImg src={ item.imgMobile } name={ item.name } aspect={ "aspect-[9/19.5]" }/>
						</div>
